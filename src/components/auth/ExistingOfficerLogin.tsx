...
export const ExistingOfficerLogin: React.FC<ExistingOfficerLoginProps> = ({
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      // Determine active service and prefix
      const currentService = service || 'Indian Army';
      const cleanServiceId = (serviceId || 'IC-7K42P9').trim().toUpperCase();

      // Lookup officer in database or initialize dynamic session officer
      let officer = findOfficerByServiceId(cleanServiceId);
      if (!officer) {
        // Create session officer so authentication never gets stuck on custom input
        officer = {
          id: `usr-${Date.now()}`,
          ...
          roleTitle: `${rank || 'Brigadier'} — Strategic Command`,
          passwordHash: password || 'Vajra@2026',
          securityPin: securityPin || '849201',
      ...
      // All initial credentials verified -> Proceed to Biometric sequence
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4" id="existing-officer-login-form">
      ...
      <button
        ...
        className="w-full py-3.5 px-6 rounded-xl text-xs font-bold tracking-widest text-slate-950 bg-gradient-to-r from-amber-400 via-cyan-400 to-emerald-400 hover:from-amber-300 hover:to-emerald-300 active:scale-[0.98] transition shadow-lg shadow-cyan-500/25 flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer select-none"
        id="login-submit-btn"
      >
        {isValidating ? (
          <>
